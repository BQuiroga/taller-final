...
agent.add_auth('localhost:3000',user,psw)


bands = JSON.parse agent.get('localhost:3000/bands.json').body
if bands.size == 1
	...
	while flag<=(limit) do	
		...
		date=Time.now
		user_id=agent.get('localhost:3000/band_informations/new').form_with(:method => 'POST').field_with(:name => 'band_information[user_id]').value
		band_id=agent.get('localhost:3000/band_informations/new').form_with(:method => 'POST').field_with(:name => 'band_information[band_id]').value
		...
		agent.post('localhost:3000/band_informations.json', data) 
		puts "Minute #{flag}"
		flag+=1
		alarms= JSON.parse agent.get('localhost:3000/alarms.json').body
		alarms.each do |alarm|
			if alarm? alarm
				Pusher.url = "http://90b3bfd4d3713a081765:def286f353288afd72b7@api.pusherapp.com/apps/126255"
				channel='test_channel'+agent.get('localhost:3000/bands/new').form_with(:method => 'POST').field_with(:name => 'band[user_id]').value
